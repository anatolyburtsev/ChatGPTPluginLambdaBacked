...
def handler(event, context):
    logger.info("Handler launched")
    logger.info(event)
    query = event["queryStringParameters"]["query"]
    load_env_vars()
    # llm = ChatOpenAI(model_name="gpt-3.5-turbo")
    # search = GoogleSearchAPIWrapper()
    logger.info(f'OPEN AI KEY PREFIX: {os.environ["OPENAI_API_KEY"][:5]}')
...
def get_links(llm: ChatOpenAI, search: GoogleSearchAPIWrapper, idea: str) -> list:
    ...
    if type(best_ideas_list[0]) == str or "link" not in best_ideas_list[0].keys():
        logger.error(
            f"GPT provided shrank list from google response: {best_ideas_list}"
        )
        return []
